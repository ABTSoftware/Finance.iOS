...
//
// AnnotationDragListener.swift is part of SCICHART®, High Performance Scientific Charts
...
class AnnotationDragListener: ISCIAnnotationDragListener {
    private let action: (ISCIAnnotation) -> Void
    init(action: @escaping (ISCIAnnotation) -> Void) {
        self.action = action
    }
